...
theme_set(
  ggthemes::theme_base() +
      theme(
        text = element_text(family = "Ubuntu Regular", color = "#22211d",  size = 22),
...
#Tema tabelas do pacote Reatable
...
plot7 <- ggplot(data = data_plot4, mapping = aes(fire_number, fire_month)) +
  geom_col(aes(color = fire_number, fill = fire_number), width = 0.8)+
  scale_color_gradient(low = "yellow", high = "red")+
  scale_fill_gradient(low = "yellow", high = "red")+
  labs(
...
plot9 <- ggplot(data = data_plot5, mapping = aes(fire_number, name_state)) +
  geom_col(aes(color = fire_number, fill = fire_number), width = 0.8)+
  scale_color_gradient(low = "yellow", high = "red")+
  scale_fill_gradient(low = "yellow", high = "red")+
  labs(
...
plot10 <- ggplot(data = data_plot6, mapping = aes(fire_number, fire_month)) +
  geom_col(aes(color = fire_number, fill = fire_number), width = 0.8)+
  scale_color_gradient(low = "yellow", high = "red")+
  scale_fill_gradient(low = "yellow", high = "red")+
  labs(
...
Neste projeto trabalhamos com dados oficiais do governo brasileiro sobre queimadas do ano de 1998 a 2017. Foi possível treinar a confecção de mapas com os pacotes geobr e plotly. Podemos ver que a partir de uma base de dados tratada ([Tidy data](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html)) fomos capazes de extrair as informações necessárias dos dados. Como atividade complementar a este projeto confeccionei no Tableau uma Dashboard que pode ser acessada [aqui](https://public.tableau.com/app/profile/tales.gomes/viz/FocosdeincndiosnoBrasil1998a2017/FocosdeincndionoBrasil).
